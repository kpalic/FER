...
@cuda.jit
def count_primes_regular(numbers, result, chunk_size):
    start = cuda.grid(1)
    ...
    cuda.atomic.add(result, 0, local_count)
...
def main():
    ...
    G = 4096  # Example global work size
    L = 128   # Example local work size
    chunk_size = N // G  # Size of the chunk each thread will process
    
    # Launch kernel
    count_primes_regular[G, L](d_numbers, d_result, chunk_size)
    cuda.synchronize()
