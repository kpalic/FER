...
// Implementacija metoda klase Neuron
Neuron::Neuron() {
    this->bias = 0;
}
...
double Neuron::computeOutput(vector<double> input, bool lastLayer) {
...
Neuron::Neuron(Neuron parentOne, Neuron parentTwo) {
...
// Implementacija metoda klase Layer
Layer::Layer() {
    this->neurons = {};
}

Layer::Layer(int numberOfNeurons, bool applicationLayer, int numberOfInputs) {
...
Layer::Layer(Layer parentOne, Layer parentTwo) {
    vector<Neuron> neurons;
...
vector<double> Layer::computeOutputs(vector<double> inputs) {
...
// Implementacija metoda klase NeuralNet

NeuralNet::NeuralNet() {
    this->layers = {};
}

NeuralNet::NeuralNet(vector<int> numberOfNeuronsInLayer, int numberOfInputs) {
    vector<Layer> layers;
...
NeuralNet::NeuralNet(NeuralNet parentOne, NeuralNet parentTwo) {
    vector<Layer> layers;
...
double NeuralNet::computeOutput(vector<double> inputs) {
...
pair<NeuralNet, NeuralNet> selection (vector<pair<NeuralNet, double>> evaluation) {
    ...
    for (auto i : evaluation) {
        probabilities.push_back(1 - (i.second / sumErrors));
    }
...
vector<NeuralNet> GeneticAlgorithm(vector<int> architecture, 
    ...
    for (int i = 1; i <= numberOfIteration; i++) {
        ...
        while (newPopulation.size() != popsize) {
            // cout << "dodajem child: population=" << population.size() << " newPopulation=" << newPopulation.size() << " evaluation=" << evaluation.size() << endl;
            if (!eliteNNAdded) {
