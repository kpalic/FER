...
void walkThroughElse(shared_ptr<set<Literal>> parents, int depth) {
    if (parents.get()->begin()->parentOne != nullptr) {
        cout << "(" << parents.get()->begin()->name << ") : (";
        for (auto a = parents.get()->begin()->parentOne.get()->begin(); a != parents.get()->begin()->parentOne.get()->end(); a++) {
            cout << ((*a).negated ? "~" : "") << (*a).name;
        }
        cout << ") (";
        for (auto a = parents.get()->begin()->parentTwo.get()->begin(); a != parents.get()->begin()->parentTwo.get()->end(); a++) {
            cout << ((*a).negated ? "~" : "") << (*a).name;
        }
        cout << ")" << endl;
        walkThroughElse(parents.get()->begin()->parentOne, depth + 1);
        walkThroughElse(parents.get()->begin()->parentTwo, depth + 1);
    }
}
...
void walkthroughResult(Literal literal, int depth = 0) {
    ...
    cout << "(" << literal.name << ") : (";
    for (auto a = literal.parentOne.get()->begin(); a != literal.parentOne.get()->begin(); a++) {
        cout << ((*a).negated ? "~" : "") << (*a).name;
    }
    cout << ") (";
    for (auto a = literal.parentTwo.get()->begin(); a != literal.parentTwo.get()->begin(); a++) {
        cout << ((*a).negated ? "~" : "") << (*a).name;
    }
    cout << ")" << endl;
