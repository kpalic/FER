...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    """
    # Prilagođavanje izračuna gradijenata težina kako bi se transponiranje uskladilo s oblikom self.weights
    grad_weights = (np.dot(self.inputs.T, grads)).T

    # Gradijenti pristranosti su suma gradijenata po primjerima.
    ...
    # # Oblik težina je (num_outputs, num_inputs), stoga moramo transponirati
    # # grad_weights kako bi odgovarao obliku težina
    # grad_weights = grad_weights.T  # Transponiranje da se uskladi s oblikom težina
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    relu_grad = self.inputs > 0  # Gradijent ReLU: 1 gdje je ulaz > 0, inače 0
    return grads * relu_grad  # Element-po-element množenje gradijenata
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    # Stabilizacija logita od x oduzimanjem maksimalne vrijednosti po svakom primjeru
    x_stable = x - np.max(x, axis=1, keepdims=True)

    # Softmax i cross-entropy loss
    log_sum_exp = np.log(np.sum(np.exp(x_stable), axis=1, keepdims=True))
    cross_entropy = -np.sum(y * x_stable, axis=1, keepdims=True) + log_sum_exp
  ...
  def backward_inputs(self, x, y):
    ...
    # Izračunavanje softmax
    exps = np.exp(x - np.max(x, axis=1, keepdims=True))
    softmax = exps / np.sum(exps, axis=1, keepdims=True)

    # Gradijent gubitka funkcije softmax cross entropy je jednostavno softmax(x) - y
    # Kako smo uzeli prosjek u forward passu, gradijent trebamo podijeliti s N
    grads = (softmax - y) / N
    return grads
