...
def make_predictions_knn(representations, r, k=3):
    knn = KNeighborsClassifier(n_neighbors=k)
    X = []
    y = []
    for label, reps in representations.items():
        for rep in reps:
            X.append(rep.cpu().numpy())
            y.append(label)
    knn.fit(X, y)
    return knn.predict(r.cpu().numpy().reshape(1, -1))

def evaluate_knn(model, repr, loader, device, k=3):
    model.eval()
    total = 0
    correct = 0
    for i, data in enumerate(loader):
        ...
        with torch.no_grad():
            r = model.get_features(anchor.to(device))
            r = r / torch.linalg.vector_norm(r)
        pred = make_predictions_knn(repr, r, k=k)
        correct += (pred == id.cpu().numpy()).sum()
        total += id.size(0)
    return correct / total
